...
function App() {
  ...
  const [cardStyle, setCardStyle] = React.useState(() => { try { return { ativo: 0, inativo: 1, ...(JSON.parse(localStorage.getItem(STYLE_KEY)) || {}) }; } catch (e) { return { ativo: 0, inativo: 1 }; } });
  const [panelOpen, setPanelOpen] = React.useState(() => { try { return localStorage.getItem("sol_tweaks_open") !== "0"; } catch (e) { return true; } });
  const refs = React.useRef({});

  const changeMode = (m) => { setMode(m); try { localStorage.setItem(MODE_KEY, m); } catch (e) {} };
  const changeCardStyle = (s) => { setCardStyle(s); try { localStorage.setItem(STYLE_KEY, JSON.stringify(s)); } catch (e) {} };
  const changePanel = (v) => { setPanelOpen(v); try { localStorage.setItem("sol_tweaks_open", v ? "1" : "0"); } catch (e) {} };
  ...
  const panelActive = panelOpen && !modalOpen;
  return React.createElement(SKINS.CTX.Provider, { value: cardStyle },
    React.createElement("div", { style: { minHeight: "100vh", background: "var(--surface-app)" } },
      ...
      React.createElement("main", { style: { padding: `76px ${panelActive ? 376 : 44}px 128px 64px`, display: "flex", flexDirection: "column", gap: 40, transition: "padding var(--dur-base) var(--ease-out)" } },
      ...
      !modalOpen && React.createElement(CardStyleTweaks, { value: cardStyle, onChange: changeCardStyle, open: panelOpen, onOpenChange: changePanel })
    )
  );
}
